...
os.environ["WANDB_API_KEY"] = 'a2cf7b7f5b49b5d67331a8002187ab2849e553e0' # 将引号内的+替换成自己在wandb上的一串值
...
from dataloader import get_meta_split_data_loaders
...
def ini_model(restore=False, restore_from=None):
    ...
    # model_f = my_net(modelname='mydeeplabV3P')
    # model_f = model_f.to(device)
    # model_f.device = device
    # model_f = nn.DataParallel(model_f, device_ids=gpus, output_device=gpus[0])
...
def train_one_epoch(model_l, model_r, niters_per_epoch, label_dataloader, unlabel_dataloader_0, unlabel_dataloader_1, optimizer_r, optimizer_l, cross_criterion, epoch):
    ...
    for idx in pbar:
        ...
        unsup_imgs_mixed = unsup_imgs_1
        # unlabeled r mixed images
        aug_unsup_imgs_mixed = aug_unsup_imgs_1
        ...
        logits_cons_tea_1 = logits_u1_tea_1

        _, ps_label_1 = torch.max(logits_cons_tea_1, dim=1)
        ps_label_1 = ps_label_1.long()

        logits_cons_tea_2 = logits_u1_tea_2
        ...
        logits_cons_stu_1, rep_unsup_l = model_l(unsup_imgs_mixed)
        aug_logits_cons_stu_1, rep_unsup_teacher = model_l(aug_unsup_imgs_mixed)
        # Get student_r prediction for mixed image
        logits_cons_stu_2, rep_unsup_r = model_r(unsup_imgs_mixed)
        ...
        var_lr, _ = cal_variance(logits_cons_stu_1, aug_logits_cons_stu_2)
        var_rl, _ = cal_variance(logits_cons_stu_2, aug_logits_cons_stu_1)
        lr_loss = torch.mean(var_lr) + torch.mean(var_rl)
        ...
        pre_sup_l, rep_l = model_l(imgs)  # tensor(16,2,288,288)   tensoe(16,2048)
        pre_sup_r, rep_r = model_r(imgs)

        pre_aug_sup_l, _ = model_l(aug_imgs)
        pre_aug_sup_r, _ = model_r(aug_imgs)
        ...
        if epoch <= 20:
            ...
            # loss = loss_l + loss_r + loss_aug_l + loss_aug_r + cps_loss + lr_loss
            # loss = loss_l + loss_r + loss_aug_l + loss_aug_r + cps_loss
            loss = loss_l + loss_r + cps_loss + lr_loss
        ...
        total_lr_loss.append(lr_loss.item())
    ...
    total_lr_loss = sum(total_lr_loss) / len(total_lr_loss)
    total_con_loss = sum(total_con_loss) / len(total_con_loss)

    return model_l, model_r, total_loss, total_loss_l, total_loss_r, total_cps_loss, total_lr_loss, total_con_loss
...
def train(label_loader, unlabel_loader_0, unlabel_loader_1, test_loader, val_loader, learning_rate, weight_decay, num_epoch, model_path, niters_per_epoch):
    ...
    for epoch in range(num_epoch):
        ...
        # normal images
        model_l, model_r, total_loss, total_loss_l, total_loss_r, total_cps_loss, total_lr_loss, total_con_loss = train_one_epoch(
        ...
        print(
            f"[ Normal image Train | {epoch + 1:03d}/{num_epoch:03d} ] total_loss = {total_loss:.5f} total_loss_l = {total_loss_l:.5f} total_loss_r = {total_loss_r:.5f} total_cps_loss = {total_cps_loss:.5f} total_lr_loss = {total_lr_loss:.5f}")
        ...
        wandb.log({'epoch': epoch + 1, 'loss/total_loss': total_loss, 'loss/total_loss_l': total_loss_l,
                  'loss/total_loss_r': total_loss_r, 'loss/total_cps_loss': total_cps_loss, 'loss/total_lr_loss': total_lr_loss,
