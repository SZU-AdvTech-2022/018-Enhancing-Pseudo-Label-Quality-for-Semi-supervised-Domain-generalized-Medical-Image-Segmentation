...
def draw_many_img(model_path_l, model_path_r, test_loader):
    ...
    for minibatch in tqdm(test_loader):
        ...
        imgs = imgs.to(device)
        mask = mask.to(device)
        if path_img[0][-8: -7] == flag:
            image_slice = path_img[0][-7:-5]
            with torch.no_grad():
                logits_l, _ = model_l(imgs)
                logits_r, _ = model_r(imgs)
...
def main():
    ...
    model_path_l = './tmodel/' + 'l_' + 'MMS_deeplab_2%_D_CM' + '.pt'
    model_path_r = './tmodel/' + 'r_' + 'MMS_deeplab_2%_D_CM' + '.pt'
    ...
    draw_img(model_path_l, model_path_r, val_loader, test_vendor)
